...
class WildberriesParser:
    ...
    def create_realistic_demo_data(self):
        ...
        brands = ["BOSCO", "Gloria Jeans", "Ostin", "Sela", "Zarina", "Mango", "Incotex", "Colin's", "befree"]
        materials = [
            "шерсть 100%", 
        ...
        sizes = ["42,44,46,48", "44,46,48,50", "40,42,44,46,48", "46,48,50,52", "42,44,46,48,50,52"]
        ...
        for i in range(25):
            ...
            product = {
                ...
                'characteristics': json.dumps({
                    ...
                    'Стиль': random.choice(['классический', 'приталенный', 'прямой']),
                    'Застежка': random.choice(['пуговицы', 'молния', 'потайная молния']),
...
def filter_products(products):
    ...
    for product in products:
        try:
            ...
            if (rating >= 4.5 and 
                price <= 10000 and 
                any(rus_word in country for rus_word in ['россия', 'russia', 'ru'])):
...
def main():
    ...
    if filtered_products:
        save_to_excel(filtered_products, 'отфильтрованный_каталог.xlsx', 'Отфильтрованные товары')
        ...
        for i, product in enumerate(filtered_products[:3]):
            print(f"   {i+1}. {product['name'][:40]}... - {product['price']} руб. Рейтинг: {product['rating']}")
    else:
        print("Нет товаров, соответствующих критериям фильтрации")
    
    create_readme_file(len(products_data), len(filtered_products) if filtered_products else 0)
